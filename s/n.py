...
def elimination_monitoring_points(self, armor_p, armor_r):
    ...
    try:
        ...
        # Esperar por la pregunta "¿Es este el archivo correcto? (s/n)"
        idx = process.expect(["¿Es este el archivo correcto? (s/n)", pexpect.EOF, pexpect.TIMEOUT])

        if idx == 0:
            # Si la pregunta fue recibida, enviar la respuesta del usuario
            user_response = armor_p.json_body["confirmation"] if "confirmation" in armor_p.json_body else "s"
            process.sendline(user_response)
        elif idx == 1:
            # Manejar EOF (fin del archivo)
